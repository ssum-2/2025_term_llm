...
class ExtractedAssertion(BaseModel):
    ...
    source_entity: str = Field("작성자", description="이 주장을 펼치는 주체. 명시된 기관/회사명(예: '하나증권')이 없으면 '작성자'라고 표기.")
...
class EntityAnalysis(BaseModel):
    ...
    entity_name: str = Field(..., description="분석 주체의 이름 (예: 'BNK투자증권', '한국은행')")
    main_stance: str = Field(..., description="이 주체가 제시하는 여러 주장을 종합한 핵심 입장 요약 (1-2 문장)")
    key_claims: List[KeyClaim] = Field(..., description="이 주체가 제시하는 주요 주장 목록 (최대 3개)")

class ReportSummary(BaseModel):
    """UI에 표시될 최종 PDF 분석 결과"""
    overall_insight: str = Field(..., description="모든 분석 주체들의 의견을 종합한 최종 인사이트 (3-4 문장)")
...
class GrandSummary(BaseModel):
    ...
    title: str = Field(..., description="종합 요약의 제목 (예: 'SK하이닉스 투자 분석 종합')")
    content: str = Field(..., description="PDF와 뉴스 정보를 종합하여 생성한 최종 분석 내용 (3-5 문장)")
...
def build_extractor_chain(llm: BaseChatModel):
    ...
    prompt = PromptTemplate(
        template=(
            "당신은 편견 없는 최고의 금융 분석가입니다. 주어진 텍스트에서 객관적인 '핵심 주장(Assertion)'과 그 근거를 추출해야 합니다.\n"
            "**[추출 지시사항]**\n"
...
def build_entity_summary_chain(llm: BaseChatModel):
    ...
    **[지시 사항]**
    ...
    2. 제시된 모든 주장을 종합하여, 이 주체의 **핵심 입장(main_stance)**을 1~2 문장으로 요약해주세요.
...
def build_news_summary_chain(llm: BaseChatModel):
    ...
    # 뉴스 요약 체인은 이제 Pydantic 모델에서 articles 필드를 제외한 부분만 생성합니다.
    # articles는 원본 데이터이므로 LLM이 생성할 필요가 없습니다.
...
2.  분석의 핵심 내용을 담은 `title`을 작성해주세요. (예: 'OO전자, 기회와 위협 요인 종합 분석')
